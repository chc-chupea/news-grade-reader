...
export default function Home() {
  ...
  return <main>
    <header className="topbar"><div className="brand"><span>読</span><div>新聞をわかりやすく<small>NEWS READER FOR STUDENTS</small></div></div><div className="version">新聞読解AI <b>Ver.3.7</b></div></header>
...
function Scanner({ onRead }: { onRead: (text: string) => void }) {
  ...
  const read = async () => {
    ...
    try {
      ...
      const orderMessage = organizedTotal ? " OpenAIが元画像を見て、見出し・本文・段組み・前段からの続きを確認しました。" : "";
