...
export default function ArticleEditor({ text, onChange, source, disabled }: {
  ...
  const selectLine = (id: string) => {
    ...
    setMessage("右の文章の同じところを選んだよ。ちがう文字があれば直してね。");
  };
  return <div className="article-editor">
    <div className={source ? "comparison-grid" : ""}>
      {source && <div className="article-reference">
        <div className="reference-toolbar"><b>記事の写真</b><label>拡大 <select aria-label="記事画像の拡大率" value={zoom} onChange={(e) => setZoom(Number(e.target.value))}><option value={1}>1倍</option><option value={2}>2倍</option><option value={3}>3倍</option></select></label></div>
        <div className="reference-scroll">
          <svg role="group" aria-label="記事の写真。文字の行を選ぶと右の文章で確認できます" viewBox={`0 0 ${source.width} ${source.height}`} style={{ width: `${zoom * 100}%`, maxWidth: "none", display: "block" }}>
        ...
        <small>左の文字を押すと、右の同じところが選ばれます。</small>
      </div>}
      <div className="article-text"><label htmlFor="article-text">直す文章</label>
